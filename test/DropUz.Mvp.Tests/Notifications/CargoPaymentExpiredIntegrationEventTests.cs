...
{
    [Fact]
    public async Task ConsumerUsesInboxToCreateCargoExpiredNotificationOnce()
    {
        DateTime expiredAtUtc = new(2026, 06, 28, 19, 0, 0, DateTimeKind.Utc);
        var repository = new InMemoryMainRepository();
        var inbox = new InMemoryIntegrationEventInbox();
        var handler = new CargoPaymentExpiredIntegrationEventHandler(
            repository,
            new InMemoryNotificationService(repository, new TestDateTimeProvider(expiredAtUtc)),
            inbox);
        var integrationEvent = new CargoPaymentExpiredIntegrationEvent(
            SourceEventId: Guid.NewGuid(),
            OrderId: Guid.NewGuid(),
            UserId: Guid.NewGuid(),
            OrderNumber: "DUZ-EXPIRED-2",
